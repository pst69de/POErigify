...
class Rig:
    ...
    def make_mechanics(self):
        ...
        print('super_stretcher.make_mechanics')
        ...
        for ix in range(self.bbone_elements):
            ...
            print('super_stretcher.make_mechanics make %03d %s' % (ix, mch_name) )
    ...
    def make_controls(self):
        ...
        print('super_stretcher.make_controls')
        bpy.ops.object.mode_set(mode ='EDIT')
        org_bones = self.org_bones
        ctrl_chain = []
    ...
    def make_tweaks(self, mch_chain):
        ...
        print('super_stretcher.make_tweaks')
        bpy.ops.object.mode_set(mode ='EDIT')
        eb = self.obj.data.edit_bones
        tweak_chain = []
    ...
    def make_deform(self, mch_chain):
        ...
        print('super_stretcher.make_deform')
        bpy.ops.object.mode_set(mode ='EDIT')
        eb = self.obj.data.edit_bones
        def_chain = []
        org_bones = self.org_bones
        ...
        # special in here: provide segmentation to ORG-shaft
        eb[self.stretch_bone].bbone_segments = self.bbone_elements
    ...
    def make_constraints(self, all_bones):
        ...
        print('super_stretcher.make_constraints')
        ...
        if self.guide_in_bone:
            print('super_stretcher.make_constraints %s stretch_to %s' % (org_bones[1],org_bones[2]))
            con = pb[org_bones[1]].constraints.new('STRETCH_TO')
            con.target = self.obj
            con.subtarget = org_bones[2]
            # provide B-Bone also to the shaft
            pb[org_bones[1]].use_bbone_custom_handles = True
            print('super_stretcher.make_constraints %s handle_in %s' % (org_bones[1],org_bones[0]))
            pb[org_bones[1]].bbone_custom_handle_start = pb[org_bones[0]]
            print('super_stretcher.make_constraints %s handle_out %s' % (org_bones[1],org_bones[2]))
            pb[org_bones[1]].bbone_custom_handle_end = pb[org_bones[2]]
        else:
            print('super_stretcher.make_constraints %s stretch_to %s' % (org_bones[0],org_bones[1]))
            con = pb[org_bones[0]].constraints.new('STRETCH_TO')
            con.target = self.obj
            con.subtarget = org_bones[1]
            # provide B-Bone also to the shaft
            pb[org_bones[0]].use_bbone_custom_handles = True
            # no handle_in in this case
            print('super_stretcher.make_constraints %s handle_out %s' % (org_bones[0],org_bones[1]))
            pb[org_bones[0]].bbone_custom_handle_end = pb[org_bones[1]]
        ixfrom = (1 if self.guide_in_deform else 0)
        for deform in deforms[ixfrom:-1]:
            target_bone = deforms[ deforms.index( deform ) + 1 ]
            print('super_stretcher.make_constraints %s stretch_to %s' % (deform,target_bone))
            con = pb[deform].constraints.new('STRETCH_TO')
            con.target = self.obj
            ...
            if deforms.index( deform ) > 0:
                ...
                print('super_stretcher.make_constraints %s handle_in %s' % (deform,from_bone))
                pb[deform].bbone_custom_handle_start = pb[from_bone]
            print('super_stretcher.make_constraints %s handle_out %s' % (deform,target_bone))
            pb[deform].bbone_custom_handle_end = pb[target_bone]
        if not self.guide_out_deform:
            print('super_stretcher.make_constraints %s stretch_to %s' % (deforms[-1],org_bones[-1]))
    ...
    def generate(self):
        ...
        print('super_stretcher.generate')
        bpy.ops.object.mode_set(mode ='EDIT')
        eb = self.obj.data.edit_bones
